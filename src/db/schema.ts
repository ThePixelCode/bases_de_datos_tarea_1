...
export const gameToTagsTable = pgTable(
  "GAMETOTAGS",
  {
...
export const playerToGameTable = pgTable(
  "PLAYERTOGAMES",
  {
    id: integer().primaryKey().generatedAlwaysAsIdentity(),
    gameId: integer("game_id")
  ...
  (table) => [
    check(
      "review",
      sql`(${table.review} IS NULL AND ${table.recommended} IS NULL) OR (${table.review} IS NOT NULL AND ${table.recommended} IS NOT NULL)`,
    ),
  ],
);

export const playerToGameRelations = relations(
  playerToGameTable,
  ({ one, many }) => ({
...
export const savedPlayTable = pgTable("SAVEDPLAYS", {
  id: integer().primaryKey().generatedAlwaysAsIdentity(),
  gameToPlayerId: integer("game_to_player_id")
    .notNull()
...
// This is not actually a friendship is more like an acknowledge
export const friendshipTable = pgTable(
  "FRIENDSHIP",
  {
    id: integer().primaryKey().generatedAlwaysAsIdentity(),
    user1: integer("user1_id")
      .notNull()
      .references(() => playerTable.id, {
        onDelete: "cascade",
        onUpdate: "cascade",
  ...
  (table) => [
    check(
      "messages",
      sql`validate_messages(${table.messages}, (ARRAY[${table.user1}::TEXT, ${table.user2}::TEXT])::TEXT[])`,
    ),
